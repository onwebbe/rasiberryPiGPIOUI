...
class GPIOPin extends React.Component {
  ...
  render() {
    ...
    if (this.props.pinObject.pinStatus == 0) {
      pinBackColor = 'green'
    } else {
      pinBackColor = 'red                                                                                                                                                                                                                                                                                                                                                                   '
    }
    var label = <div style={{display: 'inline-block'}}>{this.props.pinObject.Name}</div>
    return (
      <div style={{display: 'flex', flexFlow: 'row', justifyContent: this.state.position == 'left'?'flex-end':'flex-start' }}>
        ...
        <div style={{width: '20px', height: '20px', margin: '3px 10px', backgroundColor: pinBackColor}}>
          <span style={{color: 'white'}}>{this.props.pinObject.mode.substring(0,2)}</span>
        </div>
